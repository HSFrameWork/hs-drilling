...
function MAIN:EventHander()
    ...
    RegisterNetEvent('hs-drilling:server:Drill', function(index, product)
    ...
    RegisterNetEvent('hs-drilling:server:leavePlatform', function(index)
        self.Platforms[index]:Leave(source)
    end)
end

RegisterServerEvent("hs-drilling:server:sellstuff")
AddEventHandler("hs-drilling:server:sellstuff", function(item, qty)
    local src = source
