...
RegisterNetEvent('hs-drilling:startDrilling', function()
    pressed = true
end)
...
function PLATFORM:NUIHandler()
    ...
    RegisterNUICallback('Drill', function(data, cb)
        if self.active then
            TriggerServerEvent('hs-drilling:server:Drill', self.index, data.product)
        end
...
function PLATFORM:Close()
    ...
    self.active = false
    self:DestroyCam()
    TriggerServerEvent('hs-drilling:server:leavePlatform', self.index)
end
...
function MAIN:EventHander()
    RegisterNetEvent('hs-drilling:client:syncPlatform', function(platformData)
