...
RegisterNetEvent('hs-drilling:sellsomething')
AddEventHandler('hs-drilling:sellsomething', function()
    local newdx = {}
    ...
    if dialog ~= nil then
        if tonumber(dialog['amount']) > 0 then
            TriggerServerEvent('hs-drilling:server:sellstuff', dialog['item'], tonumber(dialog['amount']))
        end
    end
end)
...
function PLATFORM:Open()
    if not self.active then
        ...
        QBCore.Functions.TriggerCallback('hs-drilling:callback:getPlatformData',function(data)
    ...
    exports['qb-target']:AddBoxZone("oil"..k, coordinates, 5.0, 5.0, {
        ...
        maxZ= coordinates.z+5,
            ...
            options = {
                {
                    type = "client",
                    event = 'hs-drilling:startDrilling',
                    icon = "fas fa-eye",
                        ...
                        exports['qb-target']:AddBoxZone("seller", pedc, 0.70, 0.70, {
                            ...
                            maxZ=v.pedc.z+0.9,
                                ...
                                options = {
                                    {
                                        type = "client",
                                        event = 'hs-drilling:sellsomething',
                                        icon = "fas fa-eye",
                                        label = '💲 '.."Sell Items",
                                    },
                                },
			...
			if #(pedc - pedpos) < 5.0 then
                ...
                if #(pedc - pedpos) < 2.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent('hs-drilling:sellsomething')
                    end
                end
            end
		end
